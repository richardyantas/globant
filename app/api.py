...
def upload_csv_file():    
    ...
    batch = read_csv_stream(file.stream)
    
    # for i in range(0, len(csv_data), BATCH_SIZE_LIMIT):
    #     batch = csv_data[i:i + BATCH_SIZE_LIMIT]

    # for i in range(0, len(csv_data), BATCH_SIZE_LIMIT):
    #     batch = csv_data[i:i + BATCH_SIZE_LIMIT]
    
    if file.filename == "departments.csv":        
        insert_data_into_database(batch, Department)
    elif file.filename == "jobs.csv":        
        insert_data_into_database(batch, Job)        
    else:        
        insert_data_into_database(batch, Employee)
    return jsonify({'message': 'Data uploaded successfully'}), 201
